...
const Card = ({category ,id ,price ,title ,image}:Product) => {
  ...
  return (
    <div className='w-full p-4 gap-2 flex justify-between items-center flex-col min-h-96 border  border-neutral-200 rounded-2xl'>
      <Image src={image} width={600} height={600}  alt={title} className='w-full h-80 border border-neutral-200 rounded-2xl object-cover' />
      <div className="w-full h-44  ">
        ...
        <button onClick={()=> dispatch(addToFavorite({id , price ,title}))} className='w-12 flex justify-center items-center h-12 border border-store-950/15 rounded-md '>
          <Heart/>
        </button>
      </div>
    </div>
